...
export default function HistoryDataGrid({ isServer = true, searchQuery }: Props) {
    ...
    const rows = isServer ? data?.items ?? [] : (filteredRows as AniHistoryInfo[]);

    return (
        <>
            {loading ? (
                <div className="page-state" style={{ height: '50vh' }}>
                    <Loader2
                        size={32}
                        strokeWidth={2}
                        style={{ animation: 'spin 1s linear infinite', color: 'var(--color-primary)' }}
                    />
                    <p className="page-state__desc">正在加载历史记录…</p>
                </div>
            ) : (
                <div
                    key={`page-${paginationModel.page}-${data?.total ?? 0}`}
